...
today = date.today()
firstDay = today - timedelta(days=2)
tokenId = input("Coingecko token ID: ")
startDate = firstDay.strftime("%d-%m-%Y")
...
while startDate != endDate:
    ...
    list_row_append = [[startDate.format('DD-MM-YYYY'), historicalMcap, historicalVolume, historicalPrice]]
    numpy_array_rows_append = numpy.array(list_row_append)
    with open(csvname, 'a') as csvfile:
        numpy.savetxt(csvfile, numpy_array_rows_append, delimiter=',', fmt='%s', comments='')
